...
async function showingTimeRequest(
  method: string,
  path: string,
  body?: unknown,
) {
...
server.tool(
  "request_showing",
  "Request a property showing for a buyer. Uses ShowingTime if configured, falls back to GHL calendar.",
  {
  ...
  async ({
    ...
    try {
      ...
      const startISO = new Date(
        `${requestedDate}T${new Date(`1970-01-01 ${requestedTime}`).toTimeString().slice(0, 5)}`,
      ).toISOString();
...
server.tool(
  ...
  "Check available showing times for a property on a given date",
  {
    mlsId: z.string(),
  ...
  async ({ mlsId, date }) => {
    try {
      if (USE_SHOWINGTIME) {
        const data = await showingTimeRequest(
          "GET",
          `/listings/${mlsId}/availability?date=${date}`,
        );
      ...
      return mcpSuccess({
        ...
        date,
        suggestion:
          "Contact the listing agent directly or book via GHL calendar.",
      });
    } catch (e) {
      return mcpError(e);
    }
...
server.tool(
  "list_upcoming_showings",
  "List all upcoming showings for a contact or for all leads",
  {
  ...
  async ({ contactId, startDate, endDate }) => {
    try {
      const data = await ghl(
        "GET",
        `/appointments/?locationId=${GHL_LOCATION_ID}&calendarId=${GHL_CALENDAR_ID}${contactId ? `&contactId=${contactId}` : ""}${startDate ? `&startDate=${startDate}` : ""}${endDate ? `&endDate=${endDate}` : ""}`,
      );
      return mcpSuccess(data);
    } catch (e) {
      return mcpError(e);
...
server.tool(
  "cancel_showing",
  "Cancel a scheduled showing and notify the contact",
  {
...
server.tool(
  "reschedule_showing",
  "Reschedule an existing showing to a new date and time",
  {
    appointmentId: z.string(),
    contactId: z.string(),
  ...
  async ({ appointmentId, contactId, newDate, newTime, propertyAddress }) => {
    try {
      const startISO = new Date(
        `${newDate}T${new Date(`1970-01-01 ${newTime}`).toTimeString().slice(0, 5)}`,
      ).toISOString();
      const endISO = new Date(
        new Date(startISO).getTime() + 60 * 60 * 1000,
      ).toISOString();
