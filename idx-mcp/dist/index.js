...
if (!IDX_API_KEY) {
    console.error("ERROR: IDX_API_KEY environment variable is required.\n" +
...
const AUTH_HEADER = IDX_PROVIDER === "simplyrets"
    ? "Basic " +
        Buffer.from(`${IDX_API_KEY}:${IDX_API_SECRET ?? ""}`).toString("base64")
    : `Bearer ${IDX_API_KEY}`;
async function idxRequest(path, params) {
...
// ── Listing Search ────────────────────────────────────────────────────────────
server.tool("search_listings", "Search MLS listings by location, price, beds, baths, and property type", {
...
}, async ({ city, postalCode, minPrice, maxPrice, minBeds, maxBeds, minBaths, propertyType, limit, sortBy, }) => {
    try {
...
server.tool("get_listing", "Get full details for a specific MLS listing by MLS number or listing ID", { mlsId: z.string().describe("MLS number or listing ID") }, async ({ mlsId }) => {
    try {
...
server.tool("get_comparables", "Find comparable sold listings near a subject property (for CMAs and AI context)", {
...
}, async ({ city, radiusKm, minPrice, maxPrice, minBeds, soldWithinDays, limit, }) => {
    try {
...
server.tool("get_new_listings", "Get listings added in the last N days — used for lead reactivation triggers", {
    city: z.string().optional(),
    postalCode: z.string().optional(),
    minPrice: z.number().optional(),
    maxPrice: z.number().optional(),
...
}, async ({ city, postalCode, minPrice, maxPrice, minBeds, propertyType, withinDays, limit, }) => {
    try {
        const cutoff = new Date();
...
server.tool("get_price_reductions", "Find listings with recent price reductions — used to re-engage dormant leads", {
    city: z.string().optional(),
    minPrice: z.number().optional(),
    maxPrice: z.number().optional(),
    minBeds: z.number().optional(),
...
server.tool("get_open_houses", "Find upcoming open houses in an area", {
    city: z.string().optional(),
...
server.tool("get_market_stats", "Get market statistics for an area (avg price, days on market, list-to-sale ratio)", {
    city: z.string(),
...
// ── Prompts ───────────────────────────────────────────────────────────────────
server.prompt("listing_summary", "Generate a natural language summary of a listing for SMS or conversation use", { mlsId: z.string() }, ({ mlsId }) => ({
    messages: [
        {
            role: "user",
            content: {
