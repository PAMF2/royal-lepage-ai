...
const IDX_AUTH = "Basic " + Buffer.from(`${IDX_API_KEY}:${IDX_API_SECRET}`).toString("base64");
async function idxGet(path, params) {
    ...
    if (!res.ok)
        throw new Error(`IDX ${res.status}`);
    return res.json();
}
...
// ── Offer Drafting ─────────────────────────────────────────────────────────────
server.tool("draft_offer", "Draft a real estate offer letter for a buyer based on listing and buyer details", {
    ...
    conditions: z
        .array(z.string())
        .optional()
        .describe("Conditions (e.g. 'financing', 'home inspection', 'sale of buyer property')"),
    inclusions: z
        .string()
        .optional()
...
}, async ({ mlsId, buyerName, offerPrice, depositAmount, closingDate, conditions, inclusions, agentName, brokerageName, province, }) => {
    try {
        ...
        const draft = await claude(`You are a real estate agent at ${brokerageName} in ${province}, Canada. Draft a professional offer letter for the following transaction.
...
Write a complete, professional offer letter. Include all standard clauses for ${province}. Format it clearly with sections. Use formal language.`, 2500);
...
server.tool("generate_cma", "Generate a Comparative Market Analysis (CMA) for a property or seller consultation", {
    ...
    agentName: z.string().optional(),
}, async ({ subjectAddress, city, beds, baths, sqft, propertyType, sellerName, agentName, }) => {
    try {
        ...
        const cma = await claude(`You are a real estate agent at Royal LePage preparing a CMA for a seller.
...
${JSON.stringify(activeListings
            ...
            baths: l.property?.bathrooms,
        })), null, 2)}

Recent Sales (last 90 days):
${JSON.stringify(soldListings
            .slice(0, 6)
            .map((l) => ({
            address: l.address?.full,
...
Format clearly with sections. Be specific and data-driven.`, 2500);
...
server.tool("generate_listing_presentation", "Generate a listing presentation script and talking points for a seller meeting", {
    sellerName: z.string(),
    ...
    marketCondition: z
        ...
        .default("balanced"),
}, async ({ sellerName, propertyAddress, city, agentName, agentYearsExperience, suggestedPrice, marketCondition, }) => {
    try {
        const presentation = await claude(`Create a compelling listing presentation for the following meeting:
...
// ── Marketing Materials ────────────────────────────────────────────────────────
server.tool("generate_listing_description", "Write a compelling MLS listing description for a property", {
    mlsId: z
        .string()
        .optional()
    ...
    highlights: z
        .string()
        .optional()
        .describe("Key features to emphasize (e.g. 'renovated kitchen, backyard pool, quiet street')"),
    targetBuyer: z
        .string()
        .optional()
        .describe("Who this home is ideal for (e.g. 'young families, investors, downsizers')"),
    tone: z
        .enum(["luxury", "family", "investment", "standard"])
        .default("standard"),
}, async ({ mlsId, address, beds, baths, sqft, highlights, targetBuyer, tone, }) => {
    try {
        ...
        const description = await claude(`Write a compelling MLS listing description for this property.
...
Then write a shorter 80-character social media caption for Instagram/Facebook.`, 800);
...
server.tool("generate_flyer_copy", "Write copy for a property marketing flyer or brochure", {
...
}, async ({ mlsId, address, price, beds, baths, sqft, highlights, agentName, agentPhone, agentEmail, openHouseDate, }) => {
    try {
        const copy = await claude(`Write flyer copy for a real estate property listing at Royal LePage.
...
Format clearly. Keep it clean and professional.`, 800);
...
// ── Home Evaluation ────────────────────────────────────────────────────────────
server.tool("generate_home_evaluation", "Generate an AI home evaluation report with estimated value range", {
    address: z.string(),
    city: z.string(),
    beds: z.number(),
    baths: z.number(),
...
}, async ({ address, city, beds, baths, sqft, yearBuilt, condition, propertyType, ownerName, }) => {
    try {
        const cutoff = new Date();
        cutoff.setDate(cutoff.getDate() - 90);
        ...
        const evaluation = await claude(`Generate a home evaluation report for:
...
${JSON.stringify(comps.slice(0, 8).map((l) => ({
            address: l.address?.full,
            soldPrice: l.listPrice,
            beds: l.property?.bedrooms,
            baths: l.property?.bathrooms,
...
Be specific and data-driven. Professional tone.`, 2000);
